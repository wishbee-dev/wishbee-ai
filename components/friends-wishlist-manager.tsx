...
export default function FriendsWishlistManager() {
    ...
    return (
      <div className="flex items-center justify-center py-20">
        <div className="text-center">
          <RefreshCw className="w-12 h-12 text-[#DAA520] animate-spin mx-auto mb-4" />
          <p className="text-[#654321] text-lg font-medium">Loading friends' wishlists...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-50 via-orange-50 to-rose-50 p-4 sm:p-6 md:p-8">
      <div className="max-w-6xl mx-auto">
        <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-2xl border-4 border-[#DAA520] p-6 sm:p-8">
          <div className="flex flex-col sm:flex-row items-start gap-3 sm:gap-4">
            <div className="p-2 sm:p-3 bg-gradient-to-r from-amber-500 via-orange-500 to-rose-500 rounded-lg sm:rounded-xl shadow-lg">
              <Brain className="w-6 h-6 sm:w-8 sm:h-8 text-white" />
            </div>
            <div className="flex-1">
              <h3 className="text-lg sm:text-xl font-bold text-[#654321] mb-2">AI-Powered Gift Intelligence</h3>
              <ul className="space-y-1.5 sm:space-y-2 text-xs sm:text-sm text-[#8B4513]">
                ...
                <li className="flex items-center gap-2">
                  <TrendingUp className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-amber-600 flex-shrink-0" />
                  <span>Contribution recommendations based on budget and timeline</span>
                </li>
                <li className="flex items-center gap-2">
                  <Heart className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-amber-600 flex-shrink-0" />
        ...
        <div className="bg-white rounded-2xl shadow-2xl border-4 border-[#DAA520] p-6 sm:p-8 mt-6 sm:mt-8">
          <div className="flex flex-col gap-3 sm:gap-4 mb-4 sm:mb-6">
            <div className="flex-1 relative">
              <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 sm:w-5 sm:h-5 text-[#8B5A3C]" />
              <input
                type="text"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                placeholder="Search friends..."
                className="w-full pl-9 sm:pl-10 pr-3 sm:pr-4 py-2.5 sm:py-3 text-sm sm:text-base border-2 border-[#DAA520]/30 rounded-lg focus:outline-none focus:border-[#DAA520] transition-colors text-[#654321]"
              />
            </div>
            <div className="w-full sm:w-auto">
              <select
                ...
                className="w-full px-3 sm:px-4 py-2.5 sm:py-3 text-sm sm:text-base border-2 border-[#DAA520]/30 rounded-lg focus:outline-none focus:border-[#DAA520] transition-colors text-[#654321]"
              >
          ...
          <div className="grid gap-4 sm:gap-6">
            {filteredFriends.map((friend) => (
              <div
                key={friend.id}
                className="border-2 border-[#DAA520]/20 rounded-lg sm:rounded-xl p-4 sm:p-6 hover:border-[#DAA520] hover:shadow-lg transition-all"
              >
                <div className="flex flex-col sm:flex-row items-center sm:items-start gap-3 sm:gap-4 mb-4">
                  <Image
                    ...
                    width={60}
                    height={60}
                    className="sm:w-20 sm:h-20 rounded-full border-3 sm:border-4 border-[#DAA520]"
                  />
                  <div className="flex-1 text-center sm:text-left w-full">
                    <div className="flex flex-col sm:flex-row items-center sm:items-center sm:justify-between gap-2 mb-2">
                      <h3 className="text-xl sm:text-2xl font-bold text-[#654321]">{friend.friendName}</h3>
                      {friend.upcomingOccasion && (
                        <div
                          className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-full text-xs sm:text-sm font-bold whitespace-nowrap ${"bg-gradient-to-r from-amber-400 to-orange-400 text-[#654321]"}`}
                        >
                          {friend.upcomingOccasion.type} in {friend.upcomingOccasion.daysLeft} days
                        </div>
                      )}
                    </div>
                    <div className="flex items-center justify-center sm:justify-start gap-3 sm:gap-4 text-xs sm:text-sm text-[#8B4513]">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 mb-4">
                  {friend.items.map((item) => (
                    <div key={item.id} className="bg-[#F5F1E8] rounded-lg p-3 sm:p-4 border border-[#DAA520]/20">
                      <div className="flex gap-3 sm:gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <h4 className="font-bold text-sm sm:text-base text-[#654321] mb-3 line-clamp-2 h-10">
                            {item.productName}
                          </h4>
                          <p className="text-xs sm:text-sm text-[#8B4513] mb-2">${item.price.toFixed(2)}</p>
                          <div className="mb-2">
                            <div className="flex justify-between text-[10px] sm:text-xs text-[#8B4513] mb-1">
                              <span>
                          ...
                          <Button
                            ...
                            className="w-full px-3 sm:px-4 py-1.5 sm:py-2 bg-gradient-to-r from-[#DAA520] to-[#F4C430] text-[#3B2F0F] rounded-lg font-bold text-xs sm:text-sm hover:shadow-lg hover:scale-105 transition-all"
                          >
                            Contribute
                          </Button>
                        </div>
                ...
                <div className="mt-4 flex justify-end">
                  <Button
                    ...
                    className="w-full px-4 sm:px-6 py-2.5 sm:py-3 text-sm sm:text-base bg-gradient-to-r from-[#DAA520] to-[#F4C430] text-[#3B2F0F] rounded-full font-bold hover:shadow-xl hover:scale-105 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                  >
          ...
          {aiInsights && selectedFriend && (
            <div className="mt-6 sm:mt-8 bg-gradient-to-r from-amber-50 via-orange-50 to-rose-50 border-2 border-amber-200 rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-lg">
              <h3 className="text-lg sm:text-2xl font-bold text-[#654321] mb-3 sm:mb-4 flex items-center gap-2">
                <Sparkles className="w-5 h-5 sm:w-6 sm:h-6 text-amber-600" />
                <span className="text-base sm:text-2xl">AI Insights for {selectedFriend.friendName}</span>
              </h3>
              <div className="space-y-3 sm:space-y-4">
                <div className="bg-white rounded-lg p-3 sm:p-4 border-2 border-amber-200">
                  <h4 className="font-bold text-sm sm:text-base text-[#654321] mb-2">Recommended Contribution</h4>
                  <p className="text-xs sm:text-sm text-[#8B4513] mb-3">{aiInsights.reasoning}</p>
                  <div className="flex items-center gap-2 sm:gap-3">
                    <span className="text-xl sm:text-2xl font-bold text-[#DAA520]">
                      ${aiInsights.suggestedContribution}
                    </span>
                    <span className="text-xs sm:text-sm text-[#8B4513]">suggested contribution</span>
                  </div>
                </div>
                <div className="bg-white rounded-lg p-3 sm:p-4 border-2 border-amber-200">
                  <h4 className="font-bold text-sm sm:text-base text-[#654321] mb-2">Personalized Message</h4>
                  <p className="text-xs sm:text-sm text-[#8B4513] italic">{aiInsights.personalizedMessage}</p>
                </div>
              </div>
            </div>
          )}
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                    ...
                    if (preset <= remaining) {
                      return (
                        <Button
                          ...
                          className="px-3 py-1 text-sm bg-[#F5F1E8] border border-[#DAA520]/30 rounded-lg hover:bg-[#DAA520]/10 transition-colors text-[#654321]"
                        >
                          ${preset}
                        </Button>
                      )
              ...
              <div>
                <div className="flex justify-between items-center mb-2">
                  ...
                  <Button
                    ...
                    className="px-3 py-1.5 bg-gradient-to-r from-amber-500 via-orange-500 to-rose-500 text-white text-xs rounded-lg font-semibold hover:shadow-lg hover:scale-105 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1.5 border border-amber-400/30 shadow-md shadow-amber-500/20"
                  >
                    {isGeneratingMessage ? (
                      <>
            ...
            <div className="p-6 border-t border-[#DAA520]/20 flex gap-3">
              <Button
                ...
                className="flex-1 px-6 py-3 bg-gradient-to-r from-[#DAA520] to-[#F4C430] text-[#3B2F0F] rounded-lg font-bold hover:shadow-lg hover:scale-105 transition-all disabled:opacity-50"
              >
                Cancel
              </Button>
              <Button
                ...
                className="flex-1 px-6 py-3 bg-gradient-to-r from-[#DAA520] to-[#F4C430] text-[#3B2F0F] rounded-lg font-bold hover:shadow-lg hover:scale-105 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
                {isContributing ? (
                  <>
